...
diffnet_eval_path = './diffnet_eval.txt'
...
failure_dict = get_pred_dict(diffnet_eval_path)
...
for k1,v1 in failure_dict.items():
    for k2,v2 in failure_gt_dict.items():
        if k1 == k2:
            ...
            cv.namedWindow('diff-net', cv.WINDOW_NORMAL | cv.WINDOW_KEEPRATIO)
            cv.imshow('diff-net',img)
            cv.waitKey(0)
